...
class TranslationWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.progress_updated.emit(f"Завантаження документа до проекту {self.project_id}...")
            
            with open(temp_file_path, 'rb') as file:
                files = {'file': (f'source_text_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt', file, 'text/plain')}
                doc_response = self.api_client.project.attach_document(self.project_id, files)
            
            if doc_response.status_code != 200:
                raise Exception(f"Помилка завантаження документа: {doc_response.status_code} - {doc_response.text}")
            
            doc_data = doc_response.json()
            if not doc_data or len(doc_data) == 0:
                raise Exception("Не вдалося отримати інформацію про завантажений документ")
                
            document_id = doc_data[0].get('id') if isinstance(doc_data, list) else doc_data.get('id')
            ...
            while attempt < self.max_retries:
                ...
                try:
                    ...
                    if project_status.status_code == 200:
                        ...
                        self.progress_updated.emit(f"Прогрес перекладу: {progress}% (спроба {attempt}/{self.max_retries})")
                        
                        if progress >= 100:
                            break
                    else:
                        self.progress_updated.emit(f"Перевірка статусу... Спроба {attempt}/{self.max_retries}")
                except Exception as e:
                    self.progress_updated.emit(f"Помилка перевірки статусу: {str(e)} (спроба {attempt}/{self.max_retries})")

            # Крок 4: Експорт перекладу
            self.progress_updated.emit("Запит експорту перекладу...")
            export_response = self.api_client.document.request_export([document_id], target_type="target")
            
            if export_response.status_code != 200:
                raise Exception(f"Помилка запиту експорту: {export_response.status_code} - {export_response.text}")
            ...
            while export_attempts < max_export_attempts:
                ...
                self.progress_updated.emit(f"Завантаження результату... Спроба {export_attempts}/{max_export_attempts}")
...
"""
                        ...
                        raise Exception(f"Помилка завантаження: {download_response.status_code}")
                        
                except Exception as e:
                    if export_attempts >= max_export_attempts:
                        raise Exception(f"Не вдалося завантажити результат після {max_export_attempts} спроб: {str(e)}")
                    continue
...
class SmartCATGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        input_group = QGroupBox(f"Текст для перекладу ({self.source_lang.upper()} → {self.target_lang.upper()})")
        input_layout = QVBoxLayout()
        
        self.text_input = QTextEdit()
        self.text_input.setPlaceholderText("Введіть російський текст для перекладу на англійську...")
        ...
        self.translate_btn.setStyleSheet("QPushButton { font-size: 14px; padding: 8px; }")
...
🔤 Мовна пара: {self.source_lang.upper()} → {self.target_lang.upper()}
    ...
    def refresh_config(self):
        ...
        self.load_env_config()
        self.update_config_display()
        self.connection_status.setText("Статус: Конфігурацію оновлено. Необхідно перепідключитися.")
        self.connection_status.setStyleSheet("color: orange")
        self.translate_btn.setEnabled(False)
        QMessageBox.information(self, "Конфігурація", "Конфігурацію оновлено з .env файлу!")
    ...
    def connect_to_api(self):
        ...
        if not self.username or not self.password:
            QMessageBox.warning(self, "Помилка", "Будь ласка, встановіть SMARTCAT_USERNAME та SMARTCAT_PASSWORD у .env файлі")
            return
            
        if not self.project_id:
            QMessageBox.warning(self, "Помилка", "Будь ласка, встановіть SMARTCAT_PROJECT_ID у .env файлі")
            return
        ...
        try:
            ...
            if test_response.status_code == 200:
                ...
                self.connection_status.setText(f"Статус: ✅ Підключено до проекту '{project_name}'")
                self.connection_status.setStyleSheet("color: green")
                self.translate_btn.setEnabled(True)
                QMessageBox.information(self, "Успіх", f"Успішно підключено до проекту:\n{project_name}")
            else:
                raise Exception(f"Проект з ID {self.project_id} не знайдено або немає доступу: {test_response.status_code}")
                
        except Exception as e:
            self.connection_status.setText("Статус: ❌ Помилка підключення")
            self.connection_status.setStyleSheet("color: red")
            QMessageBox.critical(self, "Помилка підключення", f"Не вдалося підключитися до API:\n{str(e)}")
    ...
    def start_translation(self):
        ...
        if not source_text:
            QMessageBox.warning(self, "Помилка", "Будь ласка, введіть текст для перекладу")
            return
    ...
    def translation_finished(self, result):
        ...
        QMessageBox.information(self, "Успіх", "Переклад завершено успішно!\nФайли збережено в папці translations/")
